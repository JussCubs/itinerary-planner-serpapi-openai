...
def get_questions():
    """
    Generate 3 user-friendly questions about the trip (no mention of code or JSON).
    """
    system_prompt = "You are a friendly travel assistant helping someone plan a trip to Maui."
    user_prompt = (
        "Please provide three short, thoughtful questions to learn about someone's preferences "
        "for a Maui vacation. Respond only with a JSON array of exactly three strings, no extra text."
    )
    try:
...
def generate_itinerary(user_answers, trip_start, trip_end, all_search_data):
    """
    Always regenerate a day-by-day itinerary even if answers/dates haven't changed,
    so the user can see a new itinerary each time. 
    """
    system_prompt = (
        "You create day-by-day Maui travel plans. Be warm, descriptive, and user-friendly. "
        "No disclaimers or code references, just a single text-based plan."
    )
...
st.markdown("Create a customized day-by-day plan for an unforgettable trip to Maui!")

# Date inputs (no immediate error)
...
with st.form("maui_form"):
    ...
    plan_button = st.form_submit_button("Plan My Maui Adventure")

    if plan_button:
        if start_date_val > end_date_val:
            st.error("Please make sure your start date is before your end date.")
        else:
            # Build a key to see if we've done this exact scenario
            answers_tuple = (
                user_answer1.strip(),
                user_answer2.strip(),
                user_answer3.strip(),
            ...
            if answers_tuple not in st.session_state.cached_search_data:
                ...
                st.session_state.cached_search_data[answers_tuple] = new_data

            # 2) Always generate a fresh itinerary
            st.info("Creating your custom itinerary...")
            search_data_for_ai = st.session_state.cached_search_data[answers_tuple]
